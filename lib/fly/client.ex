defmodule Fly.Client do
  ...
  defp client, do: Application.get_env(:fly_together, :client_api)
  ...
  def fetch_organizations(args, config) do
    """
      query {
        organizations {
          nodes {
    ...
    """
    |> perform_query(args, config, :organizations)
    |> handle_response()
    # |> TemplateOptions.transform()
  end

  def create_app(args, config) do
    """
    ...
    """
    |> perform_query(%{input: args}, config, :create_app)
    |> handle_response()
    |> case do
      {:ok, %{"createApp" => %{"app" => %{"id" => id}}}} ->
        Logger.info("App created: #{inspect(id)}")
        {:ok, id}
      ...
      other ->
        Logger.error(
          "Expected result from create_app not found. Response: #{inspect(other)}"
        )
  ...
  def set_secrets(args, config) do
    """
    ...
    """
    |> perform_query(%{input: args}, config, :set_secrets)
    |> handle_response()
    |> case do
      {:ok, %{"setSecrets" => %{"app" => %{"id" => id}}}} ->
        Logger.info("App secrets updated: #{inspect(id)}")
        {:ok, id}
      ...
      other ->
        Logger.error(
          "Expected result from set_secrets not found. Response: #{inspect(other)}"
        )
  ...
  def deploy_image(args, config) do
    """
    ...
    """
    |> perform_query(%{input: args}, config, :deploy_image)
    |> handle_response()
    |> case do
      {:ok, %{"deployImage" => %{"release" => %{"id" => id}}}} ->
        Logger.info("App released: #{inspect(id)}")
        {:ok, id}
      ...
      other ->
        Logger.error(
          "Expected result from deploy_image not found. Response: #{inspect(other)}"
        )
